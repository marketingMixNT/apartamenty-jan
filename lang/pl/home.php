<?php

return [

    'meta-title' => "Hotel Jan Kraków – Komfortowy Nocleg w Centrum Starego Miasta",
    'meta-desc' => "Hotel Jan w Krakowie – stylowe pokoje 50 metrów od Rynku Głównego. Komfort, gościnność i klimat Starego Miasta w samym sercu Krakowa.",

    'slider' => [
        'subheading' => 'Inny niż wszystkie!',
        'book' => 'Rezerwuj',
    ],

    'about' => [
        'heading' => 'Gościnność i wygoda',
        'heading-2' => ' na starym mieście',
        'text-1' => "Hotel Jan to połączenie historii i nowoczesności. Mieści się w zabytkowej, 600-letniej kamienicy, oferując gościom wyjątkowy klimat Starego Miasta oraz wygodę dostosowaną do współczesnych potrzeb. Nasz hotel znajduje się zaledwie 50 metrów od Rynku Głównego.",
        'text-2' => "Oferujemy komfortowe pokoje, przestronne lobby i oranżerię, w której serwujemy śniadania. Nasza obsługa działa 24/7, aby zapewnić naszym gościom pełną satysfakcję i komfortowy pobyt w samym sercu Krakowa.",
        'link' => "dowiedz się więcej",
    ],

    'features' => [
        'title-1' => "Darmowe WiFi",
        'title-2' => "Pyszne śniadania",
        'title-3' => "Świetna lokalizacja",
        'title-4' => "Parking dla gości",
        'text-1' => "Szybki internet w dzisiejszych czasach to podstawa. Dzięki darmowemu WiFi możesz być u nas online.",
        'text-2' => "Zapraszamy na pełnowartościowe i bogate śniadania w formie bufetu.",
        'text-3' => "W samym sercu Starego Miasta, 50 m od Rynku Głównego.",
        'text-4' => "Parking czynny 24h oraz strzeżony 24h odległość 450m (5min pieszo).",
    ],

    'rooms' => [
        'heading' => "Prawdziwy krakowski urok",
        'text' => "Nasze apartamenty to połączenie elegancji i komfortu, zlokalizowane w samym sercu Krakowa. Każdy z nich urządzony jest z dbałością o szczegóły, aby zapewnić naszym gościom wyjątkowy pobyt. Z okien roztacza się widok na zabytkowe uliczki, a bliskość Rynku Głównego sprawia, że Kraków jest na wyciągnięcie ręki. Oferujemy przestrzeń do relaksu, nowoczesne udogodnienia oraz prawdziwą krakowską atmosferę.",
    ],

    'cta' => [
        'heading'=>"Zarezerwuj swój wymarzony pobyt",
        'text'=>"Odkryj wyjątkowy klimat Krakowa, relaksując się w naszych stylowych apartamentach.",
        'book'=>'Zarezerwuj'
    ],
    'local-attractions' => [
        'heading'=>"Lokalne atrakcje w zasięgu ręki",
        'text'=>"Pobyt w Hotelu Jan to doskonała okazja, aby odkryć urok Krakowa. Tuż obok znajdziesz zabytkową Starówkę, majestatyczny Zamek Wawelski i urokliwe Planty. Miasto oferuje również liczne muzea i galerie, które warto odwiedzić. Ciesz się pełnią krakowskiego klimatu, mając wszystko w zasięgu ręki!",
        'link'=>'zobacz więcej'
       
    ],

];
